...
<template>
  <div 
    class="group relative bg-gray-50 p-4 rounded-md shadow-inner flex flex-col h-full"
    style="min-height: 400px;"
    @mouseenter="isHovered = true"
    @mouseleave="isHovered = false"
  >
    <!-- Loading State -->
    <div v-if="isLoading" class="flex items-center justify-center py-10 absolute inset-0 bg-gray-50 bg-opacity-75">
      <i class="fas fa-spinner fa-spin text-3xl text-green-500"></i>
      <p class="ml-3 text-gray-700">{{ loadingMessage }}</p>
    </div>
    ...
    <div v-else class="relative flex-1 flex flex-col">
      ...
      <textarea
        ...
        class="font-mono text-lg leading-relaxed w-full h-full bg-transparent border-none focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none overflow-y-auto"
      ...
      <textarea
        ...
        class="font-mono text-lg leading-relaxed w-full h-full bg-transparent border-none focus:ring-0 resize-none cursor-default overflow-y-auto"
        readonly
        :class="{ 'select-none': isTyping }"
      />

      <!-- Floating Action Buttons -->
      <div 
        v-if="canShowButtons"
        class="absolute top-2 right-2 flex items-center gap-2 transition-all duration-200 ease-in-out"
        :class="[
          isHovered && !isTyping ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4 pointer-events-none'
        ]"
      >
        <!-- Edit Mode Buttons -->
        <template v-if="isEditing">
          <button
            @click="saveEdit"
            class="flex items-center justify-center w-8 h-8 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors duration-150 shadow-sm"
          ...
          <button
            @click="cancelEdit"
            class="flex items-center justify-center w-8 h-8 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors duration-150 shadow-sm"
      ...
      <div 
        ...
        class="absolute top-12 right-2 bg-green-500 text-white text-sm rounded px-3 py-1 shadow-lg animate-fade-in"
      >
