<template>
  <nav class="bg-primary p-4 flex flex-col md:flex-row justify-center md:justify-between items-center">
    ...
    <div class="w-full flex flex-col md:flex-row justify-center md:justify-between items-center mt-4 md:mt-0">
      <!-- Use authStore.user for consistency -->
      <h1 v-if="authStore.user && route.name !== 'User'" class="text-3xl text-white text-center font-bold hover:text-muted-foreground cursor-pointer mx-auto" @click="pushToUserView">Git Commit Summarizer</h1>
      <h1 v-else class="text-3xl text-white text-center font-bold mx-auto">Git Commit Summarizer</h1>

      <div class="mt-4 md:mt-0 flex items-center">
        <div v-if="authStore.isUserAuthenticated && authStore.user" class="flex flex-col md:flex-row items-center space-y-2 md:space-y-0 md:space-x-4">
...
// import { useUserStore } from '../stores/userStore'; // Only if userStore holds distinct data not in authStore.user
import { useRouter, useRoute } from 'vue-router'
import { Button } from './ui/button' // Assuming this is your ShadCN/UI Button

const modalActive = ref(false)
const authStore = useAuthStore()
// const userStore = useUserStore(); // If used, ensure it's for data beyond authStore.user
...
const pushToUserView = () => {
  // Prefer checking authStore.user for navigation logic
  if (authStore.user) {
    router.push('/user')
  }
}
