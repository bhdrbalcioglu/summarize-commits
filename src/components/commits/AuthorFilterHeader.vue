    ...
    <DropdownMenu>
      <DropdownMenuTrigger as-child>
        <Button variant="outline" size="sm">
          Authors <ChevronDown class="ml-2 h-4 w-4" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent class="w-56">
        ...
        <div class="flex flex-col max-h-60 overflow-y-auto">
          <DropdownMenuCheckboxItem
            class="hover:cursor-pointer hover:underline hover:bg-gray-100"
            v-for="author in authorsList"
            :key="author"
            :checked="isAuthorSelected(author)"
            @update:checked="(checked) => toggleAuthor(author, checked)"
          >
            {{ author }}
          </DropdownMenuCheckboxItem>
        </div>
...
const toggleAuthor = (author: string, checked: boolean) => {
  try {
    const newSelectedAuthors = checked
...
const isAuthorSelected = computed(
  () => (author: string) => props.selectedAuthors.includes(author)
);
</script>
